...
public class ColorSelector : MonoBehaviour {
    ...
    {
        float maxInnerDist = 0.11f; // Maximum distance for inner square

        // Restrict the movement of innerCursor within the inner square
        if (Mathf.Abs(delta.x) <= maxInnerDist && Mathf.Abs(delta.y) <= maxInnerDist)
        {
            Vector3 colorVector = new Vector3(selectedColor.r, selectedColor.g, selectedColor.b);
            Vector3 finalColorVector = (delta.x + maxInnerDist) / (2 * maxInnerDist) * colorVector + (1.0f - (delta.x + maxInnerDist) / (2 * maxInnerDist)) * new Vector3(1.0f, 1.0f, 1.0f);
            finalColor = new Color(finalColorVector.x, finalColorVector.y, finalColorVector.z);

            finalColorSprite.color = finalColor;
            innerCursor.transform.localPosition = new Vector3(delta.x, delta.y, 0); 
            innerDelta = delta;
        }
        else
        {
            // If delta is outside the inner square, we limit it
            Vector2 clampedDelta = new Vector2(Mathf.Clamp(delta.x, -maxInnerDist, maxInnerDist), Mathf.Clamp(delta.y, -maxInnerDist, maxInnerDist));
            Vector3 colorVector = new Vector3(selectedColor.r, selectedColor.g, selectedColor.b);
            Vector3 finalColorVector = (clampedDelta.x + maxInnerDist) / (2 * maxInnerDist) * colorVector + (1.0f - (clampedDelta.x + maxInnerDist) / (2 * maxInnerDist)) * new Vector3(1.0f, 1.0f, 1.0f);
            finalColor = new Color(finalColorVector.x, finalColorVector.y, finalColorVector.z);

            finalColorSprite.color = finalColor;
            innerCursor.transform.localPosition = new Vector3(clampedDelta.x, clampedDelta.y, 0); 
            innerDelta = clampedDelta;
        }
    }
